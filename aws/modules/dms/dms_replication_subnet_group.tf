...
resource "aws_dms_replication_subnet_group" "dms_replication_subnet_group" {
  ...
  tags = merge(
    {
      Name = var.dms_replication_subnet_group_name != "" ? lower(var.dms_replication_subnet_group_name) : "${lower(var.name)}-dms-replication-subnet-group-${lower(var.environment)}"
    },
    var.tags
  )
