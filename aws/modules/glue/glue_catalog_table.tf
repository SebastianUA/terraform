...
resource "aws_glue_catalog_table" "glue_catalog_table" {
    ...
    name                = var.glue_catalog_table_name != "" ? var.glue_catalog_table_name : "${lower(var.name)}-glue-catalog-table-${lower(var.environment)}"
    database_name       = var.glue_catalog_table_database_name != "" && !var.enable_glue_catalog_database ? var.glue_catalog_table_database_name : element(concat(aws_glue_catalog_database.glue_catalog_database.*.id, [""]), 0)
