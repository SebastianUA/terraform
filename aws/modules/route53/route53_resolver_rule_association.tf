#---------------------------------------------------
# Create route53 resolver rule association
#---------------------------------------------------
resource "aws_route53_resolver_rule_association" "route53_resolver_rule_association" {
    ...
    name                = var.route53_resolver_rule_association_name != "" ? lower(var.route53_resolver_rule_association_name) : "${lower(var.name)}-route53_resolve-rule-association-${lower(var.environment)}"
