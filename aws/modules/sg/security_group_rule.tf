...
resource "aws_security_group_rule" "ingress_ports" {
  ...
  security_group_id = lookup(var.ingress_ports_stack[count.index], "security_group_id", (var.enable_security_group ? element(concat(aws_security_group.security_group.*.id, [""]), 0) : null))
...
resource "aws_security_group_rule" "egress_ports" {
  ...
  security_group_id = lookup(var.egress_ports_stack[count.index], "security_group_id", (var.enable_security_group ? element(concat(aws_security_group.security_group.*.id, [""]), 0) : null))
