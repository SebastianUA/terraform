...
- `bucket_name` - Name for bucket name(`  default     = ""`).
...
- `bucket_prefix` - Creates a unique bucket name beginning with the specified prefix. Conflicts with bucket.(`  default     = ""`).
...
- `mime_types` - Set mine types (`default = {}`).
