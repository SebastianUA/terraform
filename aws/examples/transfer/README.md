...
- `enable_transfer_server` - Enable transfer server usage (`default = ""`)
...
- `transfer_server_force_destroy` - (Optional) A boolean that indicates all users associated with the server should be deleted so that the Server can be destroyed without error. The default value is false. (`default = ""`)
...
- `enable_transfer_user` - Enable transfer user usage (`default = ""`)
...
- `enable_transfer_ssh_key` - Enable transfer ssh key usage (`default = ""`)
