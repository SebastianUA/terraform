...
module "elasticache_single_redis" {
    ...
    # The single cluster
    enable_elasticache_cluster              = false
    num_cache_nodes                         = 1
...
----------------------
- `name` - Name to be used on all resources as prefix (`default     = "TEST-elasticache"`).
...
- `engine_version_default` - (Optional) Version number of the cache engine to be used. The values for default (`default     = {}`)
...
- `default_ports` - Default elasticache ports (`default     = {
        redis       = 6379
        memcached   = 11211
    }`)
...
- `parameter_group_name` - Name of the parameter group associated with this cache cluster. Ex: default.redis4.0, default.memcached1.4 etc (`default     = {}`)
