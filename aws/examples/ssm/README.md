...
- `enable_ssm_activation` - Enable ssm activation usage (`default = ""`)
...
- `ssm_activation_expiration_date` - (Optional) UTC timestamp in RFC3339 format by which this activation request should expire. The default value is 24 hours from resource creation time. Terraform will only perform drift detection of its value when present in a configuration. (`default = ""`)
- `ssm_activation_description` - (Optional) The description of the resource that you want to register. (`default = ""`)
...
- `enable_ssm_association` - Enable ssm association usage (`default = ""`)
- `ssm_association_name` - The name of the SSM document to apply. (`default = ""`)
- `ssm_association_association_name` - (Optional) The descriptive name for the association. (`default = ""`)
- `ssm_association_document_version` - (Optional) The document version you want to associate with the target(s). Can be a specific version or the default version. (`default = ""`)
- `ssm_association_instance_id` - (Optional) The instance ID to apply an SSM document to. Use targets with key InstanceIds for document schema versions 2.0 and above. (`default = ""`)
- `ssm_association_parameters` - (Optional) A block of arbitrary string parameters to pass to the SSM document. (`default = ""`)
- `ssm_association_schedule_expression` - (Optional) A cron expression when the association will be applied to the target(s). (`default = ""`)
...
- `ssm_association_max_concurrency` - (Optional) The maximum number of targets allowed to run the association at the same time. You can specify a number, for example 10, or a percentage of the target set, for example 10%. (`default = ""`)
- `ssm_association_max_errors` - (Optional) The number of errors that are allowed before the system stops sending requests to run the association on additional targets. You can specify a number, for example 10, or a percentage of the target set, for example 10%. (`default = ""`)
- `ssm_association_automation_target_parameter_name` - (Optional) Specify the target for the association. This target is required for associations that use an Automation document and target resources by using rate controls. (`default = ""`)
...
- `enable_ssm_document` - Enable ssm document usage (`default = ""`)
...
- `ssm_document_document_format` - (Optional, defaults to JSON) The format of the document. Valid document types include: JSON and YAML (`default = ""`)
...
- `enable_ssm_parameter` - Enable ssm parameter usage (`default = ""`)
...
- `ssm_parameter_description` - (Optional) The description of the parameter. (`default = ""`)
- `ssm_parameter_tier` - (Optional) The tier of the parameter. If not specified, will default to Standard. Valid tiers are Standard and Advanced. For more information on parameter tiers, see the AWS SSM Parameter tier comparison and guide. (`default = ""`)
- `ssm_parameter_key_id` - (Optional) The KMS key id or arn for encrypting a SecureString. (`default = ""`)
- `ssm_parameter_overwrite` - (Optional) Overwrite an existing parameter. If not specified, will default to false if the resource has not been created by terraform to avoid overwrite of existing resource and will default to true otherwise (terraform lifecycle rules should then be used to manage the update behavior). (`default = ""`)
- `ssm_parameter_allowed_pattern` - (Optional) A regular expression used to validate the parameter value. (`default = ""`)
- `enable_ssm_patch_group` - Enable ssm patch group usage (`default = ""`)
...
- `enable_ssm_patch_baseline` - Enable ssm patch baseline usage (`default = ""`)
- `ssm_patch_baseline_name` - The name of the patch baseline. (`default = ""`)
- `ssm_patch_baseline_description` - (Optional) The description of the patch baseline. (`default = ""`)
...
- `ssm_patch_baseline_approved_patches` - (Optional) A list of explicitly approved patches for the baseline. (`default = ""`)
- `ssm_patch_baseline_rejected_patches` - (Optional) A list of rejected patches. (`default = ""`)
...
- `ssm_patch_baseline_approval_rule_enable_non_security` - (Optional) Boolean enabling the application of non-security updates. The default value is 'false'. Valid for Linux instances only. (`default = ""`)
- `ssm_patch_baseline_patch_filter_key` - "" (`default = PATCH_SET`)
- `ssm_patch_baseline_patch_filter_values` - "" (`default = ['APPLICATION']`)
- `enable_ssm_resource_data_sync` - Enable ssm resource data sync usage (`default = ""`)
...
- `s3_destination_kms_key_arn` - (Optional) ARN of an encryption key for a destination in Amazon S3. (`default = ""`)
- `s3_destination_prefix` - (Optional) Prefix for the bucket. (`default = ""`)
...
- `enable_ssm_maintenance_window` - Enable ssm maintenance window usage (`default = ""`)
...
- `ssm_maintenance_window_description` - (Optional) A description for the maintenance window. (`default = ""`)
- `ssm_maintenance_window_allow_unassociated_targets` - (Optional) Whether targets must be registered with the Maintenance Window before tasks can be defined for those targets. (`default = ""`)
- `ssm_maintenance_window_enabled` - (Optional) Whether the maintenance window is enabled. Default: true. (`default = True`)
- `ssm_maintenance_window_end_date` - (Optional) Timestamp in ISO-8601 extended format when to no longer run the maintenance window. (`default = ""`)
- `ssm_maintenance_window_schedule_timezone` - (Optional) Timezone for schedule in Internet Assigned Numbers Authority (IANA) Time Zone Database format. For example: America/Los_Angeles, etc/UTC, or Asia/Seoul. (`default = ""`)
- `ssm_maintenance_window_start_date` - (Optional) Timestamp in ISO-8601 extended format when to begin the maintenance window. (`default = ""`)
- `enable_ssm_maintenance_window_task` - Enable ssm maintenance window task (`default = ""`)
...
- `ssm_maintenance_window_task_description` - (Optional) The description of the maintenance window task. (`default = ""`)
...
- `enable_automation_parameters` - Enable automation parameters usage block (`default = ""`)
...
- `enable_lambda_parameters` - Enable lambda parameters usage block (`default = ""`)
- `lambda_parameters_client_context` - (Optional) Pass client-specific information to the Lambda function that you are invoking. (`default = ""`)
- `lambda_parameters_payload` - (Optional) JSON to provide to your Lambda function as input. (`default = ""`)
- `lambda_parameters_qualifier` - (Optional) Specify a Lambda function version or alias name. (`default = ""`)
- `enable_run_command_parameters` - Enable run command parameters usage block (`default = ""`)
- `run_command_parameters_comment` - (Optional) Information about the command(s) to execute. (`default = ""`)
- `run_command_parameters_document_hash` - (Optional) The SHA-256 or SHA-1 hash created by the system when the document was created. SHA-1 hashes have been deprecated. (`default = ""`)
- `run_command_parameters_document_hash_type` - (Optional) SHA-256 or SHA-1. SHA-1 hashes have been deprecated. Valid values: Sha256 and Sha1 (`default = ""`)
- `run_command_parameters_output_s3_bucket` - (Optional) The name of the Amazon S3 bucket. (`default = ""`)
- `run_command_parameters_output_s3_key_prefix` - (Optional) The Amazon S3 bucket subfolder. (`default = ""`)
- `run_command_parameters_service_role_arn` - (Optional) The IAM service role to assume during task execution. (`default = ""`)
- `run_command_parameters_timeout_seconds` - (Optional) If this time is reached and the command has not already started executing, it doesn't run. (`default = ""`)
- `enable_notification_config` - Enable notification config usage block (`default = ""`)
- `notification_config_notification_arn` - (Optional) An Amazon Resource Name (ARN) for a Simple Notification Service (SNS) topic. Run Command pushes notifications about command status changes to this topic. (`default = ""`)
- `notification_config_notification_events` - (Optional) The different events for which you can receive notifications. Valid values: All, InProgress, Success, TimedOut, Cancelled, and Failed (`default = ""`)
- `notification_config_notification_type` - (Optional) When specified with Command, receive notification when the status of a command changes. When specified with Invocation, for commands sent to multiple instances, receive notification on a per-instance basis when the status of a command changes. Valid values: Command and Invocation (`default = ""`)
- `enable_run_command_parameters_parameter` - Enable parameter usage for run_command_parameters block (`default = ""`)
...
- `enable_step_functions_parameters` - Enable step functions parameters usage (`default = ""`)
- `step_functions_parameters_input` - (Optional) The inputs for the STEP_FUNCTION task. (`default = ""`)
- `step_functions_parameters_name` - (Optional) The name of the STEP_FUNCTION task. (`default = ""`)
- `enable_ssm_maintenance_window_target` - Enable ssm maintenance window target usage (`default = ""`)
- `ssm_maintenance_window_target_name` - (Optional) The name of the maintenance window target. (`default = ""`)
- `ssm_maintenance_window_target_description` - (Optional) The description of the maintenance window target. (`default = ""`)
...
- `ssm_maintenance_window_target_owner_information` - (Optional) User-provided value that will be included in any CloudWatch events raised while running tasks for these targets in this Maintenance Window. (`default = ""`)
