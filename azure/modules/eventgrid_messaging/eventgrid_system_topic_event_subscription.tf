...
resource "azurerm_eventgrid_system_topic_event_subscription" "eventgrid_system_topic_event_subscription" {
  count = var.enable_eventgrid_system_topic_event_subscription ? 1 : 0

  name                = var.enable_eventgrid_system_topic_event_subscription_name != "" ? var.enable_eventgrid_system_topic_event_subscription_name : "${lower(var.name)}-eventgrid-system-topic-event-subscription-${lower(var.environment)}"
  system_topic        = var.enable_eventgrid_system_topic_event_subscription_system_topic != "" ? var.enable_eventgrid_system_topic_event_subscription_system_topic : (var.enable_eventgrid_system_topic ? azurerm_eventgrid_system_topic.eventgrid_system_topic[count.index].name : null)
  resource_group_name = var.enable_eventgrid_system_topic_event_subscription_resource_group_name

  expiration_time_utc                  = var.enable_eventgrid_system_topic_event_subscription_expiration_time_utc
  event_delivery_schema                = var.enable_eventgrid_system_topic_event_subscription_event_delivery_schema
  eventhub_endpoint_id                 = var.enable_eventgrid_system_topic_event_subscription_eventhub_endpoint_id
  hybrid_connection_endpoint_id        = var.enable_eventgrid_system_topic_event_subscription_hybrid_connection_endpoint_id
  service_bus_queue_endpoint_id        = var.enable_eventgrid_system_topic_event_subscription_service_bus_queue_endpoint_id
  service_bus_topic_endpoint_id        = var.enable_eventgrid_system_topic_event_subscription_service_bus_topic_endpoint_id
  included_event_types                 = var.enable_eventgrid_system_topic_event_subscription_included_event_types
  labels                               = var.enable_eventgrid_system_topic_event_subscription_labels
  advanced_filtering_on_arrays_enabled = var.enable_eventgrid_system_topic_event_subscription_advanced_filtering_on_arrays_enabled

  dynamic "azure_function_endpoint" {
    iterator = azure_function_endpoint
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_azure_function_endpoint)) > 0 ? [var.eventgrid_system_topic_event_subscription_azure_function_endpoint] : []
  ...
  dynamic "storage_queue_endpoint" {
    ...
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_storage_queue_endpoint)) > 0 ? [var.eventgrid_system_topic_event_subscription_storage_queue_endpoint] : []
  ...
  dynamic "webhook_endpoint" {
    ...
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_webhook_endpoint)) > 0 ? [var.eventgrid_system_topic_event_subscription_webhook_endpoint] : []
  ...
  dynamic "subject_filter" {
    ...
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_subject_filter)) > 0 ? [var.eventgrid_system_topic_event_subscription_subject_filter] : []
  ...
  dynamic "delivery_identity" {
    ...
    for_each = var.eventgrid_system_topic_event_subscription_delivery_identity
  ...
  dynamic "delivery_property" {
    ...
    for_each = var.eventgrid_system_topic_event_subscription_delivery_property
  ...
  dynamic "dead_letter_identity" {
    ...
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_dead_letter_identity)) > 0 ? [var.eventgrid_system_topic_event_subscription_dead_letter_identity] : []
  ...
  dynamic "storage_blob_dead_letter_destination" {
    ...
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_storage_blob_dead_letter_destination)) > 0 ? [var.eventgrid_system_topic_event_subscription_storage_blob_dead_letter_destination] : []
  ...
  dynamic "retry_policy" {
    ...
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_retry_policy)) > 0 ? [var.eventgrid_system_topic_event_subscription_retry_policy] : []
  ...
  dynamic "timeouts" {
    iterator = timeouts
    for_each = length(keys(var.eventgrid_system_topic_event_subscription_timeouts)) > 0 ? [var.eventgrid_system_topic_event_subscription_timeouts] : []
