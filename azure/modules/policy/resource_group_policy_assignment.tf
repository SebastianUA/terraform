...
resource "azurerm_resource_group_policy_assignment" "resource_group_policy_assignment" {
  count = var.enable_resource_group_policy_assignment ? 1 : 0

  name                 = var.resource_group_policy_assignment_name != "" ? var.resource_group_policy_assignment_name : "${lower(var.name)}-resource-group-policy-assignment-${lower(var.environment)}"
  resource_group_id    = var.resource_group_policy_assignment_resource_group_id
  policy_definition_id = var.resource_group_policy_assignment_policy_definition_id != "" ? var.resource_group_policy_assignment_policy_definition_id : (var.enable_policy_definition ? azurerm_policy_definition.policy_definition[count.index].id : null)
  ...
  dynamic "non_compliance_message" {
    ...
    for_each = var.resource_group_policy_assignment_non_compliance_message
  ...
  dynamic "timeouts" {
    iterator = timeouts
    for_each = length(keys(var.resource_group_policy_assignment_timeouts)) > 0 ? [var.resource_group_policy_assignment_policy_timeouts] : []
