...
resource "azurerm_private_endpoint" "private_endpoint" {
  ...
  dynamic "private_service_connection" {
    ...
    content {
      ...
      private_connection_resource_id    = lookup(private_service_connection.value, "private_connection_resource_id", (var.enable_private_link_service ? azurerm_private_link_service.private_link_service[0].id : null))
